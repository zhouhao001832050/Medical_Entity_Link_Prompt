...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        out, _ = self.lstm(out)  # [batch_size, seq_len, hidden_size * num_direction]=[128, 32, 256]
        out_entity,_ = self.lstm(out_entity)
        # mean pooling  # 37.15%
        # out = torch.mean(out, dim=1, keepdim=True)
        # out = torch.squeeze(out, dim=1)
        # out_entity = torch.mean(out_entity, dim=1, keepdim=True)
        # out_entity = torch.squeeze(out_entity, dim=1)

        # max pooling  # 26.9%
        # out = torch.max(out, dim=1)[0]
        # out_entity = torch.max(out_entity, dim=1)[0]

        # last pooling  # 16.65%
        out = out[:,-1,:]
        out_entity = out_entity[:,-1,:]
