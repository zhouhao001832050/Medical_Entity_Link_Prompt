...
class Config(object):
    ...
    def __init__(self, dataset, embedding):
        ...
        self.class_list = [x.strip() for x in open(
            dataset + self.model_name +'/class.txt', encoding='utf-8').readlines()]              # 类别名单
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = nn.Linear(config.hidden_size * 4, config.hidden_size2)
        self.fc = nn.Linear(config.hidden_size2, config.num_classes)

    def forward(self, x):
        # x, _ = x[0]
        emb = self.embedding(x[0])  # [batch_size, seq_len, embeding]=[128, 32, 300]
        emb_entity = self.embedding(x[1])
        H, _ = self.lstm(emb)  # [batch_size, seq_len, hidden_size * num_direction]=[128, 32, 256]
        H_entity,_ = self.lstm(emb_entity)
        M = self.tanh1(H)  # [128, 32, 256]
        M_entity = self.tanh1(H_entity)

        alpha = F.softmax(torch.matmul(M, self.w), dim=1).unsqueeze(-1)  # [128, 32, 1]
        alpha_entity = F.softmax(torch.matmul(M_entity, self.w), dim=1).unsqueeze(-1)
        out = H * alpha  # [128, 32, 256]
        out_entity = H_entity * alpha_entity
        out = torch.sum(out, 1)  # [128, 256]
        out_entity = torch.sum(out_entity, 1)
        out = F.relu(out)
        out_entity = F.relu(out_entity)
        out=torch.cat((out,out_entity),1)
        out = self.fc1(out)
        # out_entity = self.fc1(out_entity)
